<script>
    ...
    import {readDefaultConfiguration, saveConfiguration} from '../../utils/configuration';
    ...
    const resetConfiguration = async () => {
        try {
            ...
            ToastStore.addToast(ToastStore.severity.SUCCESS, 'Success resetting configuration. Restarting the app in 3 secs...');

            window.setTimeout(() => process.relaunch(), 3000);
        } catch (e) {
            console.trace(e);
            ToastStore.addToast(ToastStore.severity.ERROR, 'There was an error resetting configuration: ' + e.message);
        }
