<script>
  ...
  ToastStore.subscribe((e) => {
    ...
    if (e.length > $state.visibleToasts.length) {
      ...
      window.setTimeout(
        () => {
          $state.visibleToasts = $state.visibleToasts.slice(
            1,
...
<div class="fixed right-0 top-0 m-5 z-50">
  {#each $state.visibleToasts as [message, severity, id, ToastComponent], index (id)}
    <div in:receive={{ key: id }} out:send={{ key: id }} animate:flip>
      <ToastComponent {message} {severity} />
    </div>
  {/each}
</div>

<div
  class={$SettingsStore[SettingsStore.SETTINGS_OPTION.UI_THEME] === "dark"
    ? "dark"
    : ""}
>
  <div
    class="font-sans min-h-screen flex flex-col justify-center items-center bg-gray-50 dark:bg-gray-900 overflow-hidden dark:text-gray-300 text-gray-800"
  >
    {#if $SplashStore.length > 0}
      <svelte:component
        this={$SplashStore[$SplashStore.length - 1].component}
